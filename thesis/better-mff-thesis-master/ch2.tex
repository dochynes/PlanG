...
minimální a maximální stupeň vrcholů nebo zakázat vybrané malé podgrafy(například trojúhelníky či čtyřcykly).
...
Při rozšíření generátoru o barvy jsme narazili na následující problém. Barvy
musí ovlivňovat i test izomorfismu: dva vrcholy lze ztotožnit pouze tehdy,
pokud mají stejnou barvu. Generátor proto musí barvy znát už ve chvíli, kdy
rozhoduje, zda je nové rozšíření kanonické.

V původní implementaci existuje rychlá kontrola, která během refinementu \footnote{ Refinement je postup,
který postupně rozděluje vrcholy do menších skupin podle
jejich okolí v grafu. Vrcholy, které skončí v různých skupinách, už nemohou být
při izomorfismu zaměněny.} sleduje
pozici nově přidaného vrcholu. Pokud refinement ukáže, že tento vrchol nemůže být
kanonicky poslední, rozšíření se ihned odmítne. Po přidání barev však může nastat
situace, kdy barvy rozdělí vrcholy tak přesně, že je každý vrchol samostatně
rozlišen ještě před tím, než se tato kontrola provede. Bylo proto nutné doplnit
zvláštní případ pro úplné rozdělení vrcholů: pokud nově přidaný vrchol není na
poslední pozici kanonického pořadí, rozšíření zahodíme.
...
Tyto mechanismy jsou výkonné, ale nejsou příliš pohodlné pro běžné použití v C++.
Uživatel musí znát přesné signatury callbacků, způsob kompilace generátoru s příslušnými
makry a často také interní reprezentaci aktuálního grafu. V této práci na tyto možnosti
navazujeme, ale schováváme je za C++ funkční objekty. Uživatel pak může místo
nízkoúrovňového pluginu zapsat například predikát nad objektem reprezentujícím graf.

\section{Omezení původních rozhraní}
\label{sec:original-interface-limitations}

Původní rozhraní generatorů \texttt{geng} a \texttt{plantri} je velmi vhodné pro použití
...
V následujících kapitolách se budeme snažit zachovat rychlost původních
generátorů a jejich schopnost prořezávat výpočet během generování, ale zároveň
nad nimi nabídnout jednotné C++ rozhraní. To umožni uživateli pohodlně
nastavovat parametry, psát vlastní callbacky a pracovat s grafy bez zbytečného
kopírování dat.
